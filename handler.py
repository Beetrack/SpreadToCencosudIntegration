...
def integrate(event, context):
  ...
  if (resource == "route" and event == "update" and account_id == int(account_id_paris) and (truck in paris_trucks)):
    ...
    get_paris_route = BeetrackAPI(os.environ.get("paris_api_key"), "https://app.beetrack.com/api/external/v1").get_route(paris_route_id) 
    ...
    response = {"statusCode": 200, "body": "Message: Dispatch was updated in Paris succesfully with new status."}
    ...
    if group_name == "Paris" and (body.get("status") in [1,2,3,4]):
      ...
      response = {"statusCode": 200, "body": "Message: Dispatch was updated with new status."}
    ...
    response = {"statusCode": 200, "body": "Message: Webhook resource is not 'route' or 'dispatch'. Not doing anything"}
