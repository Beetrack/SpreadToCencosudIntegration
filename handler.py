...
def integrate(event, context):
  ...
  if (resource == "route" and event == "update" and account_id == int(account_id_paris)):
    ...
    get_paris_route = BeetrackAPI(os.environ.get("paris_api_key")).get_route(paris_route_id, account_id)
    ...
    if get_trunk_dispatches == []:
      ...
      verify_spread_truck = spread.check_or_create_trucks(truck_identifier, account_id)
      print("Verify existence Paris truck on Spread :", verify_spread_truck)
      create_trunk_route_on_spread = spread.create_new_trunk_route(verify_spread_truck, get_trunk_dispatches, account_id)
    ...
    update_dispatch_id_on_spread = spread.get_id_dispatch_spread(account_id)
    ...
    update_trunk_dispatch_on_paris = paris.update_trunk_dispatch(account_id)
    ...
    spread_route = BeetrackAPI(os.environ.get("spread_group_api_key")).get_route(spread_route_id, account_id)
    ...
    if not spread_route:
      ...
      verify_paris_truck = paris.check_or_create_trucks(truck_identifier, account_id)
      ...
      create_route_on_paris = paris.create_new_route(verify_paris_truck, get_paris_dispatches, account_id)
      ...
      start_paris_route = paris.start_route(new_paris_route_id, route_start_at, account_id)
    ...
    finish_paris_route = paris.finish_route(ended_at, route_id, int(os.environ.get("tag_route")), account_id)
